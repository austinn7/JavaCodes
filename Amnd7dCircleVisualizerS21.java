 ...
 * @author Austi
 */
public class Amnd7dCircleVisualizerS21 implements Visualizer {
    ...
    public Amnd7dCircleVisualizerS21(){
        name = "Austin's Audio Viz 2.0";
    ...
    public void setup(Integer numBands, AnchorPane vizPane){
        ...
        for (int i = 0; i < numBands; i++){
            ...
            rectangle.setY(0);
            rectangle.setRotate(720.0/numOfBands * i);
    ...
    public void visualize(double timestamp, double lenght, float[] magnitudes, float[] phases){
        ...
        for (int i = 0; i < num; i++){
            rectangles[i].setHeight((((60.0 + magnitudes[i])/30.0) * halfBandHeight));
            rectangles[i].setFill(Color.hsb(startHue - (magnitudes[i] * -6.0), 1.0, 1.0, 1.0));
        }
        
        int j = 0; 
        for (int i = num - 1; i >= num / 2; i--) {
            rectangles[i].setHeight(((60.0 + magnitudes[j])/30.0) * halfBandHeight);
